(function() {
	// Controller
	angular.module('canteen.app').controller('dish.popular.controller', [
	// Dependencies
	...
	function($scope, $http, $routeParams, $amui) {
		...
		$scope.loadPage = function() {
			$http.get(window.location.apiRoot + 'api/dish/popular/' + pageSize + '/' + pageIndex++).success(function(dishPage) {
		...
		$scope.navToDetail = function(dishId) {
			store.set('cached_popular_scope', {
		...
		var cachedPopularScope = store.get('cached_popular_scope');
		if (cachedPopularScope) {
			$scope.criteria = cachedPopularScope.criteria;
			$scope.dishes = cachedPopularScope.dishes;
			setTimeout(function() {
				$('.raty-control:not(:has("i"))').each(function() {
					var ratyControl = $(this);
					var score = ratyControl.text();
			...
			$(window).smoothScroll({
				position : cachedPopularScope.scrollPosition
			});
			store.set('cached_popular_scope', null);
		} else {
			$scope.loadPage();
		}
		$amui.load();
