(function() {
	// Controller
	angular.module('canteen.app').controller('dish.specials.controller', [
	// Dependencies
	...
	function($scope, $http, $routeParams, $amui) {
		...
		$scope.loadPage = function() {
			$http.get(window.location.apiRoot + 'api/dish/recommended/' + pageSize + '/' + pageIndex++).success(function(dishPage) {
		...
		$scope.navToDetail = function(dishId) {
			store.set('cached_recommendation_scope', {
		...
		var cachedRecommendationScope = store.get('cached_recommendation_scope');
		if (cachedRecommendationScope) {
			$scope.criteria = cachedRecommendationScope.criteria;
			$scope.dishes = cachedRecommendationScope.dishes;
			setTimeout(function() {
			...
			$(window).smoothScroll({
				position : cachedRecommendationScope.scrollPosition
			});
			store.set('cached_recommendation_scope', null);
		} else {
			$scope.loadPage();
		}
		$amui.load();
